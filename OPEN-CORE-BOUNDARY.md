...
**Status:** v0.7 — **brand consolidation edition** (2026-05-17 IST afternoon). All packages renamed from `@rocketlang/*` → `@xshieldai/*` (npm) and `*-kavachos` → `xshieldai-*` (PyPI). Same code, same versions, new umbrella under xShieldAI Posture Suite. Builds on v0.6 release-wave state; no other policy changes. See `MIGRATION.md` for the full mapping.
